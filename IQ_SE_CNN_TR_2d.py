...
# TSNetIQ
class CNNTransformer2D(nn.Module):
    def __init__(self, embed_dim=256, num_heads=8, ff_dim=512, num_layers=2,max_len=512):
        super(CNNTransformer2D, self).__init__()
        # 卷积层
        self.conv1 = nn.Conv2d(1, 16, kernel_size=(8, 7), stride=(1, 2), padding=(0, 3))
        self.bn1 = nn.BatchNorm2d(16)
        self.conv2 = nn.Conv2d(16, 32, kernel_size=(1, 5), stride=(1, 2), padding=(0, 2))
        self.bn2 = nn.BatchNorm2d(32)
        self.conv3 = nn.Conv2d(32, 64, kernel_size=(1, 3), stride=(1, 2), padding=(0, 1))
        self.bn3 = nn.BatchNorm2d(64)
        self.conv4 = nn.Conv2d(64, 128, kernel_size=(1, 3), stride=(1, 2), padding=(0, 1))
        self.bn4 = nn.BatchNorm2d(128)
        self.conv5 = nn.Conv2d(128, 256, kernel_size=(1, 1), stride=(1, 2), padding=(0, 0))
        self.bn5 = nn.BatchNorm2d(256)
        self.se5 = SEBlock2d(256)
    ...
    def forward(self, x):
        ...
        x = self.se5(F.relu(self.bn5(self.conv5(x))))
...
# Training function
def train_model(model, train_loader, val_loader, epochs=200, lr=2e-4, weight_decay=1e-3):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model.to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=weight_decay)
    scaler = GradScaler()
    lr_scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=5, min_lr=1e-6)
    ...
    df_metrics.to_csv('/home/IQ_SE_CNN_TR_M4.csv', index=False)
