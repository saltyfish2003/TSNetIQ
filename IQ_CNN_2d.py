...
class CNN2DRegressor(nn.Module):
    def __init__(self):
        super(CNN2DRegressor, self).__init__()
...
def train_model(model, train_loader, val_loader, epochs=200, lr=1e-4):
    ...
    scaler = GradScaler()
    lr_scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=5, min_lr=1e-6,threshold=5e-6)
    ...
    df_metrics.to_csv('/home/IQ_CNN_M4.csv', index=False)
